...
class TotalTermScoreTranscript(models.TransientModel):
    _name = 'total.term.transcript.wizard'
    _description = 'Total Term Score Transcript'
    ...
    def generate_ad_total_term_transcript_wizard(self):
        if not all([self.grade_id, self.company_id]):
            raise ValidationError("يرجى ملء جميع الحقول المطلوبة: المدرسة، الصف، والفصل الدراسي")
        return self.env.ref('classroom_management.action_report_ad_total_term_transcript').report_action(self)