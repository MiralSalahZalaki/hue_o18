...
class MCFollowUpRecord(models.Model):
    _name = 'mc.follow.up.record'
    ...
    syllabus_id = fields.Many2one("education.syllabus", string="Syllabus", required=True, domain="[('company_id', '=', company_id)]")
    grade_id = fields.Many2one("education.class", string="Grade", readonly=True, related="syllabus_id.class_id", domain="[('school', '=', company_id)]")
    ...
    student_list = fields.One2many('follow.up.student.list', "connect_id", string="Students")
    ...
    def unlink(self):
        for record in self:
            if record.student_list:
                record.student_list.unlink()  # Delete all related records in follow.up.student.list
        return super(MCFollowUpRecord, self).unlink()
            ...
            if rec.syllabus_id and rec.class_id:
                ...
                if syllabus_per_class:
                    ...
                    if new_student_records:
                        self.env['follow.up.student.list'].create(new_student_records)
